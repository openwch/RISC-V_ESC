...
void pause(uint16_t ms){
	TIM1->CH1CVR = 0; // volume of the beep, (duty cycle) don't go above 25
	TIM1->CH2CVR = 0;
	TIM1->CH3CVR = 0;

	delayMillis(ms);
	TIM1->CH1CVR = beep_volume; // volume of the beep, (duty cycle) don't go above 25 out of 2000
	TIM1->CH2CVR = beep_volume;
	TIM1->CH3CVR = beep_volume;
}
...
void playBJNote(uint16_t freq, uint16_t bduration){        // hz and ms
	...
	TIM1->ATRLR = timerOne_reload;
	TIM1->CH1CVR = beep_volume * timerOne_reload /TIM1_AUTORELOAD ; // volume of the beep, (duty cycle) don't go above 25 out of 2000
	TIM1->CH2CVR = beep_volume * timerOne_reload /TIM1_AUTORELOAD;
	TIM1->CH3CVR = beep_volume * timerOne_reload /TIM1_AUTORELOAD;
