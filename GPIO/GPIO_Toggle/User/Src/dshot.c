...
uint32_t gcr[37] =  {0};
volatile uint16_t dshot_frametime;
...
//{
//    if((m!=0) && (m%2 == 0)) PRINT("\n");
//    PRINT("%d ",dma_buffer[m]);
//    if((m%2) ==1) PRINT("%d ",dma_buffer[m]-dma_buffer[m-1]);
//}
//PRINT("\r\n");
//PRINT("flen:%d\r\n",dshot_frametime);


#if defined(MCU_V203F8)
	         ...
	         if((dshot_frametime < 1350 ) &&(dshot_frametime > 1150)){
				...
				if(!armed){
					if (dshot_telemetry == 0){
						 if(INPUT_PIN_PORT->INDR & INPUT_PIN){  // if the pin is high for 100 checks between signal pulses its inverted
							 high_pin_count++;
