...
#define TEMP30_CAL_VALUE            ((uint16_t*)((uint32_t)0x1FFFF720))  //ref_voltage
#define TEMP110_CAL_VALUE           ((uint16_t*)((uint32_t)0x1FFFF722))  //ref_temp
...
//set gpio pull_down
INPUT_PIN_PORT->CFGLR &= ~(0xf<<0);
INPUT_PIN_PORT->CFGLR |= (0x8<<0);
INPUT_PIN_PORT->BCR = INPUT_PIN;


delayMicros(1000);
for(int i = 0 ; i < 1000; i ++){
     if( !(INPUT_PIN_PORT->INDR & INPUT_PIN)){  // if the pin is low for 5 checks out of 100 in  100ms or more its either no signal or signal. jump to application
         low_pin_count++;
     }
     delayMicros(10);
}
...
#else
    current_state = (!(GPIOA->INDR & GPIO_Pin_2));
//    current_state = !((INPUT_PIN_PORT->INDR & INPUT_PIN));
#endif
    if (rising){
        if (current_state){
            bemfcounter++;
...
void PeriodElapsedCallback(){
            ...
            COM_TIMER->DMAINTENR &= ~((0x1UL << (0U)));             // disable interrupt
    ...
    thiszctime = INTERVAL_TIMER->CNT;
            if (rising)
            {
                for (int i = 0; i < filter_level; i++)
                {
                    ...
                    {
#else
                if( (GPIOA->INDR & GPIO_Pin_2) )     //比较器反向
//                    if((INPUT_PIN_PORT->INDR & INPUT_PIN)) //比较器反向
                    {
#endif
                        return;
                    }
            ...
            else
            {
                ...
                {
#else
                    if(!(GPIOA->INDR & GPIO_Pin_2)) //==0若检测到高电平，未过0，反电动势毛刺
//                    if(!(INPUT_PIN_PORT->INDR & INPUT_PIN))//比较器有反向
                    {
#endif
                        return;
                    }
            ...
            COM_TIMER->CNT = 0;
            COM_TIMER->ATRLR = waitTime;
            COM_TIMER->INTFR = 0x00;
            COM_TIMER->DMAINTENR |= (0x1UL << (0U));             // enable COM_TIMER interrupt
}
...
if(!prop_brake_active){
 if (zero_crosses < (20 >> stall_protection)){
    ...
    TIM1->CH1CVR = adjusted_duty_cycle;
    TIM1->CH2CVR = adjusted_duty_cycle;
    TIM1->CH3CVR = adjusted_duty_cycle;
    }
...
//{
//       NVIC_SetPriority(IC_DMA_IRQ_NAME, 0x20);
//       NVIC_SetPriority(EXTI2_IRQn, 0x30);
//}
//else
//{
//    NVIC_SetPriority(IC_DMA_IRQ_NAME, 0x30);
...
#ifdef GIMBAL_MODE
    TIM1->CH1CVR = ((2*pwmSin[phase_A_position])+gate_drive_offset)*TIMER1_MAX_ARR/2000;
    TIM1->CH2CVR = ((2*pwmSin[phase_B_position])+gate_drive_offset)*TIMER1_MAX_ARR/2000;
    TIM1->CH3CVR = ((2*pwmSin[phase_C_position])+gate_drive_offset)*TIMER1_MAX_ARR/2000;
#else
    TIM1->CH1CVR = (((2*pwmSin[phase_A_position]/SINE_DIVIDER)+gate_drive_offset)*TIMER1_MAX_ARR/2000)*sine_mode_power / 10;
    TIM1->CH2CVR = (((2*pwmSin[phase_B_position]/SINE_DIVIDER)+gate_drive_offset)*TIMER1_MAX_ARR/2000)*sine_mode_power / 10;
    TIM1->CH3CVR = (((2*pwmSin[phase_C_position]/SINE_DIVIDER)+gate_drive_offset)*TIMER1_MAX_ARR/2000)*sine_mode_power / 10;
#endif
}
...
#ifndef USE_SERIAL_TELEMETRY
//    USART_Printf_Init(115200);   //QFN20 does not support
#endif
    ...
    TIM_CCxCmd(TIM1, TIM_Channel_4,TIM_CCx_Enable); // timer used for timing adc read
    TIM1->CH4CVR = 100;                             // set in 10khz loop to match pwm cycle timed to end of pwm on
...
#else
    TIM_CCxCmd(IC_TIMER_REGISTER, TIM_Channel_3, TIM_CCx_Enable);  // input capture and output compare
    ...
    while(1)
    {
        ...
        adc_counter++;
           if(adc_counter>10){   // for adc and telemetry
               ...
               ADC_CCR = TIM1->CH3CVR*2/3 + 1;  // sample current at quarter pwm on
               if (ADC_CCR > tim1_arr){
                   ADC_CCR = tim1_arr;
               }
               TIM1->CH4CVR = ADC_CCR;
               ...
               converted_degrees = TempSensor_Volt_To_Temper(ADC_raw_temp / 4095 * 3300);
...
//               actual_current = ((smoothed_raw_current * 3300/41) - (CURRENT_OFFSET*100))/ (MILLIVOLT_PER_AMP);
     ...
     if(newinput > 2000){
         newinput = 2000;
           ...
           stuckcounter = 0;
               ...
               } //dshot==0, bi_direction == 1
               else if (dshot && bi_direction)//dshot==1, bi_direction == 1
               {
                   if (newinput > 1047) {
              ...
              if ((zero_crosses > 1000) || (adjusted_input == 0))
              {
                 ...
                 if(adjusted_input == 0 && armed){
                   GPIOB->BSHR = LL_GPIO_PIN_8; // off red
                   GPIOB->BCR = LL_GPIO_PIN_5;  // on green
                   GPIOB->BSHR = LL_GPIO_PIN_3;  //off blue
                 }
     #endif
              }
     ...
     if(input > 48 && armed){
                 ...
                 if(do_once_sinemode){
                     COM_TIMER->DMAINTENR &= ~((0x1UL << (0U)));  // disable commutation interrupt in case set
                     maskPhaseInterrupts();
                     TIM1->CH1CVR = 0;
                     TIM1->CH2CVR = 0;
                     TIM1->CH3CVR = 0;
     ...
     proportionalBrake();
         ...
         allOff();
                     ...
                     if(input > 0 && armed){
                         TIM1->CH1CVR = input;                                             // set duty cycle to 50 out of 768 to start.
                         TIM1->CH2CVR = input;
                         TIM1->CH3CVR = input;
                     }else{

                         TIM1->CH1CVR = 0;                                             // set duty cycle to 50 out of 768 to start.
                         TIM1->CH2CVR = 0;
                         TIM1->CH3CVR = 0;
                     }
     #endif
