...
void telem_UART_Init(void)
{
  ...
  */
  ...
  NVIC_SetPriority(DMA1_Channel4_IRQn, 0xE0);  //传输完成中断
...
void send_telem_DMA()
{
    // set data length and enable channel to start transfer
      MODIFY_REG(USART1->CTLR1, 0x3<<2, 0x1<<3);  //使能发送
      DMA1_Channel4->CNTR  = nbDataToTransmit;
      DMA1_Channel4->MADDR = (uint32_t)aTxBuffer;  //DMA地址自增的，需要重新设置
